...
def deconv(input_tensor, name, kw, kh, n_out, dw=2, dh=2, activation_fn=tf.nn.relu):
	n_in = input_tensor.get_shape()[-1].value
	n_b = input_tensor.get_shape()[0].value
	n_h = input_tensor.get_shape()[1].value
	n_w = input_tensor.get_shape()[2].value
	with tf.variable_scope(name):
		weights = tf.get_variable('weights', [kh, kw, n_out, n_in], tf.float32, xavier_initializer())
		biases = tf.get_variable("bias", [n_out], tf.float32, tf.constant_initializer(0.0))
		conv = tf.nn.conv2d_transpose(input_tensor, weights, [n_b, n_h*2, n_w*2, n_out], (1, dh, dw, 1), padding='SAME')
		activation = activation_fn(tf.nn.bias_add(conv, biases))
		return activation
